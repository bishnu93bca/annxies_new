...
if($show==1)
{
	...
	<title>Directory Listing of <?php echo str_replace('\\', '', dirname(strip_tags($_SERVER['PHP_SELF']))).'/';?></title>
	...
	<body>
	<div id="container">
	  <h1>Directory Listing of <?php echo $_SERVER['HTTP_HOST'];?></h1>
	  <div id="breadcrumbs"> <a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?show='.$show;?>">home</a> 
	  <?php
		...
		if(($bsize = sizeof($breadcrumbs))>0) {
			...
			for($bi=0;$bi<($bsize-1);$bi++) {
				...
				echo ' &gt; <a href="'.strip_tags($_SERVER['PHP_SELF']).'?show='.$show.'&dir='.urlencode($sofar).'">'.$breadcrumbs[$bi].'</a>';
			}
		}
	  ...
	  <div id="listingcontainer">
		...
		if($dirok) {
		?>
		<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?show='.$show.'&dir='.urlencode($dotdotdir);?>" class="<?php echo $class;?>"><img src="<?php echo $includeurl; ?>dlf/dirup.png" alt="Folder" /><strong>..</strong> <em>&nbsp;</em>&nbsp;</a></div>
		<?php
			if($class=='b') $class='w';
			else $class = 'b';
		}
		...
		for($i=0;$i<$arsize;$i++) {
		?>
		<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?show='.$show.'&dir='.urlencode(str_replace($startdir,'',$leadon).$dirs[$i]);?>" class="<?php echo $class;?>"><img src="<?php echo $includeurl; ?>dlf/folder.png" alt="<?php echo $dirs[$i];?>" /><strong><?php echo $dirs[$i];?></strong> <em>-</em> <?php echo date ("M d Y h:i:s A", filemtime($includeurl.$leadon.$dirs[$i]));?></a></div>
		<?php
			if($class=='b') $class='w';
			else $class = 'b';	
		}
		...
		for($i=0;$i<$arsize;$i++) {
			...
			if($forcedownloads) {
				$fileurl = $_SESSION['PHP_SELF'] .'?show='.$show.'&dir=' . urlencode(str_replace($startdir,'',$leadon)) . '&download=' . urlencode($files[$i]);
			}
		...
		<div id="upload">
			...
			<div id="uploadcontent">
				...
				if($phpallowuploads) {
				?>
				<form method="post" action="<?php echo strip_tags($_SERVER['PHP_SELF']);?>?show=<?php echo $show;?>&dir=<?php echo urlencode(str_replace($startdir,'',$leadon));?>" enctype="multipart/form-data">
