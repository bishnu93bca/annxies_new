    ...
    if(sizeof($fetch_details)>0)
    {
        ...
        for($row=0;$row<sizeof($fetch_details);$row++)
        {
            ...
            <li>
                ...
                <span class="last_li">
                <a href="index.php?pageType=<?php echo $pageType;?>&dtls=<?php echo $dtls;?>&contentID=<?php echo $fetch_details[$row]['contentID'];?>&moduleId=<?php echo $moduleId; ?>&dtaction=articleview&redstr=<?php echo $redirectString;?>"><img src="images/edit.gif" alt="" width="16" height="16" border="0" /></a>
                
                <a href="index.php?pageType=<?php echo $pageType;?>&dtls=<?php echo $dtls;?>&dtaction=status&action=status&stschgto=<?php echo $status?>&id=<?php echo $fetch_details[$row]['contentID'];?>&moduleId=<?php echo $moduleId; ?>&action=content&redstr=<?php echo $redirectString;?>"><?php echo $conStatus;?></a>
                <?php if($_SESSION['UTYPE']=="A") { ?>
                <a class="ask" href="index.php?pageType=<?php echo $pageType;?>&dtls=<?php echo $dtls;?>&dtaction=delete&id=<?php echo $fetch_details[$row]['contentID'];?>&action=content&moduleId=<?php echo $moduleId; ?>&redstr=<?php echo $redirectString;?>"><img src="images/delete.png" alt="" width="16" height="16" border="0" /></a></span>
            	<?php }?>
            </li>
			<?php
            $i++;
